...
[MCMC_out, ~, ~] = run_MCMC(data_vars, OFMM_params, probit_params, n_runs, burn, thin, k_max, q_dem, p_cov, alpha, eta, mu_0, Sig_0, S, sim_n, scenario);
...
[MCMC_out, OFMM_params, probit_params] = run_MCMC(data_vars, OFMM_params, probit_params, n_runs, burn, thin, k_fixed, q_dem, p_cov, alpha, eta, mu_0, Sig_0, S, sim_n, scenario);
...
function [MCMC_out, OFMM_params, probit_params] = run_MCMC(data_vars, OFMM_params, probit_params, n_runs, burn, thin, k_max, q_dem, p_cov, alpha, eta, mu_0, Sig_0, S, sim_n, scenario)
    ...
    % Save output
    save(strcat('sOFMM_MCMC_scen',num2str(scenario),'_iter',num2str(sim_n)), 'MCMC_out');
end
...
function analysis = analyze_results(post_MCMC_out, data_vars, q_dem, S, sim_n, scenario)
    ...
    save(strcat('sOFMM_Results','_scen',num2str(scenario),'_iter',num2str(sim_n)), 'post_MCMC_out', 'analysis');
end
