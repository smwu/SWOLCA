...
clear; clc;
% Specifications
num_sims = 10;                                      % Number of simulated sets
scenario = 4;                                      % Simulation scenario
model = "sOFMM";                                  % Model used
data_dir = "/n/home01/stephwu18/wsOFMM/data/";     % Input directory for data
model_dir = "/n/home01/stephwu18/wsOFMM/results/"; % Input directory for model results 
out_dir = "/n/home01/stephwu18/wsOFMM/";           % Output directory 

samp_n = 0;                                        % If using sampled data from a single iteration
iter = 1;
if samp_n > 0  % If working with sample, iterate over the samples instead of the iterations 
    num_sims = samp_n;
end 

% Initialize outputs, stored for all simulated sets
all.K = NaN(num_sims, 1);         % Initialize estimated number of classes
all.K_match = NaN(num_sims, 1);   % Initialize boolean vector of correct estimation of number of classes
all.sens = NaN(num_sims, 1);      % Initialize sensitivity
all.spec = NaN(num_sims, 1);      % Initialize specificity
all.dic6 = NaN(num_sims, 1);      % Initialize DIC version 6 to penalize overfitting 
all.aebic = NaN(num_sims, 1);     % Initialize AEBIC model selection metric
all.mse_Phi = NaN(num_sims, 1);   % Initialize MSE of class-specific response proportions
all.mse_theta = NaN(num_sims, 1); % Initialize MSE of class-specific item response probs
all.corr_S_C = NaN(num_sims, 1);  % Initialize correlation between S and C
all.corr_S_Y = NaN(num_sims, 1);  % Initialize correlation between S and Y
all.corr_C_Y = NaN(num_sims, 1);  % Initialize correlation between C and Y

for sim_n = 1:num_sims  % For each simulated set or sample
    if samp_n > 0       % If working with sample 
        
        % Check if the results file exists
        if isfile(strcat(model_dir, model, '_results_scen', num2str(scenario), '_iter', num2str(iter), '_samp', num2str(sim_n), '.mat'))
            
            % Load simulated dataset
            load(strcat(data_dir, 'simdata_scen', num2str(scenario), '_iter', num2str(iter), '_samp', num2str(sim_n), '.mat'));
            % Load model results
            load(strcat(model_dir, model, '_results_scen', num2str(scenario), '_iter', num2str(iter), '_samp', num2str(sim_n), '.mat')) 
    ...
    else  % Working with population
        
        % Check if the results file exists
        if isfile(strcat(model_dir, model, '_results_scen', num2str(scenario), '_iter', num2str(sim_n), '.mat'))  

            % Load simulated dataset
            load(strcat(data_dir, 'simdata_scen', num2str(scenario), '_iter', num2str(sim_n), '.mat'))  
            % Load wsOFMM results
            load(strcat(model_dir, model, '_results_scen', num2str(scenario), '_iter', num2str(sim_n), '.mat')) 
...
if samp_n > 0  % If working with sample dataset
    save(strcat(out_dir, 'summary_', model, '_scen', num2str(scenario), '_sample'), 'all', 'res');
else           % if working with population dataset
