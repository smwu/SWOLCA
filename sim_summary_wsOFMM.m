...
aebic_all = zeros(num_sims, 1);     % Initialize AEBIC model selection metric
...
for sim_n = 1:num_sims              % For each simulated set
    ...
    aebic_all(sim_n) = analysis.aebic;  % AEBIC model selection metric
...
res.aebic_median = [median(aebic_all, 'omitnan') quantile(aebic_all, 0.025, 2) quantile(aebic_all, 0.975, 2)];  
