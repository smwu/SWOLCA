...
%                 sum(abs(sort(samp_data.true_xi) - sort(analysis.xi_med)))
        ...
        if samp_n > 0  % If working with sample 
            save(strcat(out_dir, 'sOFMM_latent_results_scen', num2str(scenario), '_iter', num2str(sim_n), '_samp', num2str(samp_n)), 'post_MCMC_out', 'analysis', 'runtime');
        else
            save(strcat(out_dir, 'sOFMM_latent_results_scen', num2str(scenario), '_iter', num2str(sim_n)), 'post_MCMC_out', 'analysis', 'runtime');
        end    
    end    
end
