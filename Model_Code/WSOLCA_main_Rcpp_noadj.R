...
# 'WSOLCA_main_Rcpp_noadj' runs the WSOLCA model and saves and returns results 
# without applying the post-processing adjustment
# Inputs:
...
#   K_MCMC: Adaptive sampler MCMC output for K
WSOLCA_main_Rcpp_noadj <- function(data_path, adapt_path, adj_path, stan_path, 
  ...
  res <- list(analysis_adj = analysis, runtime = runtime, 
...
res_path <- paste0(wd, res_dir, model, "_results_comb_scen", scen_samp, 
...
if (already_done) {
  ...
  results_adj <- WSOLCA_main_Rcpp_noadj(data_path = data_path, adapt_path = adapt_path,
                                  adj_path = res_path, stan_path = stan_path, 
