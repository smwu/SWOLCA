...
data_dir <- "Data/Mar17/"               # Simulated data directory
res_dir <- "Results/Mar17/"             # Model results directory

# Strong patterns, overlap, 5% sample size, stratified cluster 
save_scen_metrics(scen_pop = 1212, scen_samp = 121212, WSOLCA = TRUE, 
                  SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
                  SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
                  covs = "true_Si", save_name = "metrics_scen", wd = wd, 
                  data_dir = data_dir, res_dir = res_dir)
# Strong patterns, overlap, 5% sample size, marginal, stratified with no clustering
save_name = "metrics_marg_scen"
covs <- NULL
save_scen_metrics(scen_pop = 1221, scen_samp = 122111, WSOLCA = TRUE, 
                  SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
                  SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
                  covs = covs, save_name = save_name, wd = wd, 
                  data_dir = data_dir, res_dir = res_dir)
# Strong patterns, non-overlap, 5% sample size, X~S, stratified 
save_scen_metrics(scen_pop = 3112, scen_samp = 311211, WSOLCA = TRUE, 
                  SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
                  SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
                  covs = "true_Si", save_name = "metrics_scen", wd = wd, 
                  data_dir = data_dir, res_dir = res_dir)
# Strong patterns, non-overlap, 5% sample size, X~S, stratified cluster
save_scen_metrics(scen_pop = 3112, scen_samp = 311212, WSOLCA = TRUE, 
                  SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
                  SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
                  covs = "true_Si", save_name = "metrics_scen", wd = wd, 
                  data_dir = data_dir, res_dir = res_dir)
# Weak patterns, non-overlap, 5% sample size, stratified cluster
save_scen_metrics(scen_pop = 2112, scen_samp = 211212, WSOLCA = TRUE, 
                  SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
                  SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
                  covs = "true_Si", save_name = "metrics_scen", wd = wd, 
                  data_dir = data_dir, res_dir = res_dir)
# Baseline setting, no adjustment, stratified sampling
save_name <- "metrics_scen_noadj"
save_scen_metrics(scen_pop = 1112, scen_samp = 111211, WSOLCA = TRUE, 
                  SOLCA = FALSE, WOLCA = FALSE, WSOLCA_name = "_results_comb_scen", 
                  SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
                  covs = "true_Si", save_name = save_name, wd = wd, 
                  data_dir = data_dir, res_dir = res_dir)
# Baseline setting, no adjustment, stratified cluster sampling
...
# Sampling scenarios
scenarios <- c(121212, 122111, 311211, 311212, 211212)
scen_names <- c("Overlap StrCl", "Overlap Marg", "X~S Strat", "X~S StrCl", "Weak StrCl")
save_names <- c("metrics_scen", "metrics_marg_scen", rep("metrics_scen", 3))
...
save_name <- "metrics_scen_noadj"
metrics_summ <- as.data.frame(matrix(NA, nrow = 2, ncol = 12))
colnames(metrics_summ) <- c("No Adjustment", "Model", 
...
metrics_summ[, 1] <- c("Stratified NoAdj", "Strat Clus NoAdj")
metrics_summ[, 2] <- rep("SWOLCA", 2)
output_inds <- c(1, 2, 4, 6, 3, 5, 7)
load(paste0(wd, analysis_dir, save_name, 111211, ".RData"))
metrics_summ[1, -c(1,2)] <- c(metrics_all$metrics_ws[output_inds], 
...
load(paste0(wd, analysis_dir, save_name, 111212, ".RData"))
metrics_summ[2, -c(1,2)] <- c(metrics_all$metrics_ws[output_inds], 
                              mean(metrics_all$metrics_ws$pi_cover_avg), 
                              mean(metrics_all$metrics_ws$theta_cover_avg),
                              mean(metrics_all$metrics_ws$xi_cover_avg))
metrics_summ %>% 
  kbl(digits = 3, align = "rrrrrrrrrrrr", booktabs = TRUE, format = "html",
...
# # Baseline: stratified, 5% sample size, X~S, supervised, confounder
# save_scen_metrics(scen_pop = 3212, scen_samp = 321211, WSOLCA = TRUE, 
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "true_Si", save_name = "metrics_scen")
# # Sampling schemes: stratified cluster
# save_scen_metrics(scen_pop = 3212, scen_samp = 321212, WSOLCA = TRUE, 
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "true_Si", save_name = "metrics_scen")
# # Sampling schemes: SRS
# save_scen_metrics(scen_pop = 3212, scen_samp = 321213, WSOLCA = TRUE, 
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name, 
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "true_Si", save_name = "metrics_scen")
# # Effect modifier marginal
# save_scen_metrics(scen_pop = 3222, scen_samp = 322211, WSOLCA = TRUE, 
...
# # Sample size: 10%
# save_scen_metrics(scen_pop = 3212, scen_samp = 321221, WSOLCA = TRUE,
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name,
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "true_Si", save_name = "metrics_scen")
# # Sample size: 1%
# save_scen_metrics(scen_pop = 3212, scen_samp = 321231, WSOLCA = TRUE,
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name,
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "true_Si", save_name = "metrics_scen")
# # Weak patterns
# save_scen_metrics(scen_pop = 3242, scen_samp = 324211, WSOLCA = TRUE,
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name,
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "true_Si", save_name = "metrics_scen")
# # Additional confounders
# save_scen_metrics(scen_pop = 3232, scen_samp = 323211, WSOLCA = TRUE,
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name,
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "additional", save_name = "metrics_scen")
# # No clustering
# save_scen_metrics(scen_pop = 3252, scen_samp = 325211, WSOLCA = TRUE,
#                   SOLCA = TRUE, WOLCA = TRUE, WSOLCA_name = WSOLCA_name,
#                   SOLCA_name = SOLCA_name, WOLCA_name = WOLCA_name,
#                   covs = "additional", save_name = "metrics_scen")
# 
...
wd <- "/n/holyscratch01/stephenson_lab/Users/stephwu18/SWOLCA/" # Working directory
data_dir <- "Data/Mar5/"               # Simulated data directory
res_dir <- "Results/Mar5/"             # Model results directory
analysis_dir <- "Analysis_Code/"  # Analysis directory where metrics are saved

create_table1(wd = wd, analysis_dir = analysis_dir, format = "latex", digits = 3)


#================ PLOT RMSE SUMMARY =========================================

# Sampling scenarios
scenarios <- c(321213, 321211, 321212)
...
samp_pi <- plot_rmse_boxplot(wd = wd, analysis_dir = analysis_dir, 
                             ...
                             param = "pi", upper_lim = 0.1, xlab = "Sampling Scheme", 
                             ylab = expression("RMSE for "~pi))
samp_theta <- plot_rmse_boxplot(wd = wd, analysis_dir = analysis_dir, 
                                save_names = save_names, scenarios = scenarios,
                                scen_names = scen_names, overall_name = "Sampling", 
                                param = "theta", upper_lim = 0.03, xlab = "Sampling Scheme", 
...
scenarios <- c(321211, 324211)
scen_names <- c("Mode 85%", "Mode 55%")
...
# Selection bias
scenarios <- c(321211, 322211, 113211)
...
# Pattern strength and separation
scenarios <- c(321211, 324211, 322211)
scen_names <- c("Mode 85%", "Mode 55%", "Marginal")
save_names <- c(rep("metrics_scen", 2), "metrics_marg_scen")
create_app_tables(wd, analysis_dir, save_names, scenarios, scen_names, 
                  overall_name = "Pattern", format = "html")

# Sample size
scenarios <- c(321221, 321211, 321231)
...
# 
# scenarios <- c(321211, 322211)
# scen_names <- c("Confounder \nMeasured", "Precision \nUnmeasured")
# save_names <- c("metrics_scen", "metrics_marg_scen")
# create_app_tables(wd, analysis_dir, save_names, scenarios, scen_names, 
#                   overall_name = "Selection Bias", format = "html")
...
p_theta_strat <- plot_theta_patterns(wd = wd, data_dir = data_dir, 
                                     ...
                                     scen_pop = 3212, iter_pop = 1, scen_samp = 321211,
...
p_theta_clus <- plot_theta_patterns(wd = wd, data_dir = data_dir, 
                                    analysis_dir = analysis_dir,
                                    scen_pop = 3212, iter_pop = 1, scen_samp = 321212,
                                    scen_name = "metrics_scen")
annotate_figure(p_theta_clus, 
                top = text_grob("Modal pattern per latent class, averaged across 100 iterations"))

# Investigate wrong pattern for WOLCA
# Load summary result
load(paste0(wd, analysis_dir, "metrics_marg_scen", 322211, ".RData"))
K_wrong <- which(metrics_all$metrics_unsup$K_dist != 0)
sim_res_path_full <- paste0(wd, res_dir, "wOFMM", "_results_wt_scen", 
                            322211, "_samp", K_wrong, ".RData")
...
p_pi_samp <- plot_pi_samp(wd = wd, analysis_dir = analysis_dir, scen_pop = 3212)
plot_xi_samp(wd = wd, analysis_dir = analysis_dir, scen_pop = 3212)
ggarrange(p_pi_samp, 
          p_theta_strat,
...
load(paste0(wd, analysis_dir, "metrics_scen", 321212, ".RData"))
...
samp_params <- get_avg_over_samps(wd = wd, data_dir = data_dir, scen_pop = 3222,
                                  scen_samp = 322211, iter_pop = 1, 
                                  samp_n_seq = 1:100, covs = NULL)
plot_Phi_patterns_marg(wd = wd, data_dir = data_dir, analysis_dir = analysis_dir, 
                       scen_pop = 3222, scen_samp = 322211, iter_pop = 1,
                       scen_name = "metrics_marg_scen", samp_params = samp_params)

# Stratified default, pi and xi
samp_params <- get_avg_over_samps(wd = wd, data_dir = data_dir, scen_pop = 3212,
                                  scen_samp = 321211, iter_pop = 1, 
...
plot_pi_iter(wd = wd, data_dir = data_dir, analysis_dir = analysis_dir, 
             scen_pop = 3212, scen_samp = 321211, samp_n = 91, 
             WSOLCA_name = "_results_comb_adjRcpp_scen", 
             SOLCA_name = "_results_scen", WOLCA_name = "_results_wt_scen")

# plot_theta_iter_j(wd = wd, data_dir = data_dir, analysis_dir = analysis_dir, 
#                   scen_pop = 3212, scen_samp = 321211, samp_n = 91, j = 1, 
#                   WSOLCA_name = "_results_comb_adjRcpp_scen", 
#                   SOLCA_name = "_results_scen", WOLCA_name = "_results_wt_scen")

plot_Phi_iter_marg(wd = wd, data_dir = data_dir, analysis_dir = analysis_dir,
                   scen_pop = 3222, scen_samp = 322211, samp_n = 31,
                   WSOLCA_name = "_results_comb_adjRcpp_scen", 
                   SOLCA_name = "_results_scen", WOLCA_name = "_results_wt_scen")
