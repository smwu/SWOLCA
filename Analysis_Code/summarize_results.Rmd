...
get_theta_dist <- function(est_theta, true_theta, order) {
  ...
  if (is.null(order)) {  # If no optimal ordering exists
    ...
    return(list("theta_dist" = min_dist, "order" = order, "est_theta_perm" = est_theta_perm))
...
get_MSE <- function(data_dir, res_dir, scen_pop, scen_samp, iter_pop=1, R_seq, model, 
  ...
  K_bias <- pi_bias <- theta_bias <- xi_bias <- Phi_bias <- rep(NA, R) # bias squared using posterior median
  pi_var <- theta_var <- xi_var <- Phi_var <- rep(NA, R) # posterior variance
  ...
  for (i in 1:R) { 
    ...
    if (!file.exists(sim_samp_path) | !file.exists(sim_res_path)) {
      ...
      # COMMENT OUT STAN IF NOT USING STAN MODEL
      if (adjust) {
        sim_adj_path <- paste0(res_dir, model, "_stan",  "_latent_results_adj_scen", scen_samp, "_iter", iter_pop, "_samp", samp_n, ".RData")
        if (!file.exists(sim_adj_path)) {
          print(paste0("File does not exist: ", model, "_stan",  "_latent_results_adj_scen", scen_samp, "_iter", iter_pop, "_samp", samp_n, ".RData"))
        } else {
          #print("Using adjusted file for sample ", samp_n)
          load(sim_adj_path)
          # ADDITIONAL STAN ADJUSTMENT
          analysis$k_red <- length(analysis$pi_med)
            
          # Convert to previous xi format
          dims_xi <- dim(analysis$xi_red)
          analysis$xi_red <- matrix(analysis$xi_red, nrow = dims_xi[1], ncol = dims_xi[2]*dims_xi[3])
          analysis$xi_med <- c(analysis$xi_med)
        }
      ...
      if (analysis$k_red > sim_pop$true_K) {
        ...
        if (coverage) {
          analysis$pi_red <- abind(analysis$pi_red, array(0, dim=c(M, missing)), along=2)
          analysis$theta_red <- abind(analysis$theta_red, array(0, dim=c(M, p, missing, d)), along = 3)
          if (model == "wOFMM") {
            analysis$xi_med_lb <- c(analysis$xi_med_lb, rep(0, missing*S))
            analysis$xi_med_ub <- c(analysis$xi_med_ub, rep(0, missing*S))
          } else {
            analysis$xi_red <- abind(analysis$xi_red, array(0, dim=c(M, missing*S)), along = 2)
          }
        }
      }
      ...
      K_bias[samp_n] <- (analysis$k_red - sim_pop$true_K)^2
      ...
      theta_bias[samp_n] <- theta_perm$theta_dist
      theta_var[samp_n] <- sum(var(theta_perm$est_theta_perm))
      ...
      pi_bias[samp_n] <- pi_perm$pi_dist
      pi_var[samp_n] <- sum(var(pi_perm$est_pi_perm))
      if (posthoc) {
      ...
      xi_bias[samp_n] <- xi_perm$xi_dist
      xi_var[samp_n] <- sum(var(xi_perm$est_xi_perm))
      ...
      if (coverage) {
        ...
        if (model == "wOFMM") {
          ...
          xi_CI_unord <- rbind(analysis$xi_med_lb, analysis$xi_med_ub)
          xi_CI <- xi_CI_unord
          for (s in 1:S) {
            xi_CI[, (s-1)*true_K + (1:true_K)] <- xi_CI_unord[, (s-1)*true_K + order]
          }
        } else {
          # With factor variable coding
  ...
  K_bias2 <- mean(K_bias, na.rm = TRUE)
  pi_bias2 <- mean(pi_bias, na.rm = TRUE)
  theta_bias2 <- mean(theta_bias, na.rm = TRUE)
  xi_bias2 <- mean(xi_bias, na.rm = TRUE)
  pi_var <- mean(pi_var, na.rm = TRUE)
  theta_var <- mean(theta_var, na.rm = TRUE)
  xi_var <- mean(xi_var, na.rm = TRUE)

  # List of returns
  # ret_list <- list(K_MSE = K_MSE, pi_MSE = pi_MSE, theta_MSE = theta_MSE, xi_MSE = xi_MSE)
  ret_list <- list(K_bias2 = K_bias2, 
                   pi_bias2 = pi_bias2, theta_bias2 = theta_bias2, xi_bias2 = xi_bias2, 
                   pi_var = pi_var, theta_var = theta_var, xi_var = xi_var)
  if (posthoc) {
...
MSE_SRS_ws <- get_MSE(data_dir=data_dir, res_dir=res_dir, scen_pop=1, scen_samp=101, 
                      iter_pop=iter_pop, R_seq=R_seq, model="wsOFMM", adjust=TRUE)
...
```{r}
### Create table of MSEs with bias and variance
MSE_summ <- as.data.frame(matrix(NA, nrow=6, ncol=9))
colnames(MSE_summ) <- c("Sampling Scheme", "Model", 
                        "K bias^2", "$\\pi$ bias^2", "$\\pi$ var", 
                        "$\\theta$ bias^2", "$\\theta$ var", 
                        "$\\xi$ bias^2", "$\\xi$ var")
MSE_summ[, 1] <- c(rep("SRS", 3), rep("Stratified", 3))
MSE_summ[, 2] <- rep(c("Unwtd(sOFMM)", "Wtd(wsOFMM)", "Unsup(wOFMM)"), 2)  ## latent versions
output_inds <- c(1,2,5,3,6,4,7)
MSE_summ[1, -c(1,2)] <- c(MSE_SRS_s[output_inds])
MSE_summ[2, -c(1,2)] <- c(MSE_SRS_ws[output_inds])
MSE_summ[3, -c(1,2)] <- c(MSE_SRS_unsup[output_inds])
MSE_summ[4, -c(1,2)] <- c(MSE_Strat_s[output_inds])
MSE_summ[5, -c(1,2)] <- c(MSE_Strat_ws[output_inds])
MSE_summ[6, -c(1,2)] <- c(MSE_Strat_unsup[output_inds])
MSE_summ %>% 
gt(caption = "Baseline scenario MSE of posterior parameter estimates, averaged over 100 samples") %>%
cols_label("$\\pi$ bias^2" = "π bias^2", "$\\theta$ bias^2" = "θ bias^2", "$\\xi$ bias^2" = "ξ bias^2",
           "$\\pi$ var" = "π var", "$\\theta$ var" = "θ var", "$\\xi$ var" = "ξ var") %>%
fmt_scientific(
  columns = 3:9,
  decimals = 3
) 
...
xi_plot_data <- as.data.frame(rbind(MSE_Strat_s$xi_all,  MSE_Strat_ws$xi_all, MSE_Strat_unsup$xi_all))
    # which.max(MSE_Strat_ws$xi_all[,6])
    # MSE_Strat_ws$xi_all[74,]
    # temp_Strat_ws <- MSE_Strat_ws$xi_all
    # temp_Strat_ws[74,]<- NA
    # xi_plot_data <- as.data.frame(rbind(MSE_Strat_s$xi_all, temp_Strat_ws, MSE_Strat_unsup$xi_all))
...
# Troubleshooting
load(paste0(res_dir, model, "_stan",  "_latent_results_adj_scen", scen_samp=201, "_iter", iter_pop, "_samp", samp_n=74, ".RData"))
plot(analysis$xi_red[,3,2], type="l")
plot(analysis$xi_red[,1,1], type="l")
analysis$xi_med

rstan::traceplot(analysis$out_stan, pars="xi")
plot(analysis$mcmc_permuted_ordered[,1,369], type = "l")
rstan::traceplot(analysis$out_stan, pars="pi")
apply(analysis$mcmc_permuted_ordered[, 1, 364:369], 2, median)
...
# Obtain true observed population parameters
true_params <- get_true_params(sim_pop)
xi_plot_data <- as.data.frame(rbind(MSE_SRS_s$xi_all,  MSE_SRS_ws$xi_all, MSE_SRS_unsup$xi_all))
colnames(xi_plot_data) <- paste0("xi_", 1:ncol(xi_plot_data))
xi_plot_data$Model <- c(rep("sOFMM", times=R), rep("wsOFMM", times=R), rep("wOFMM", times=R))
xi_plot_data <- xi_plot_data %>% gather("xi_component", "value", -Model)
ggplot(xi_plot_data, aes(x=xi_component, y=value, fill=Model)) +
  ...
  geom_segment(mapping=aes(x=4.5, xend=5.5, y=true_params$true_xi[5], yend=true_params$true_xi[5]),color="forestgreen") +
  geom_segment(mapping=aes(x=5.5, xend=6.5, y=true_params$true_xi[6], yend=true_params$true_xi[6]),color="forestgreen") +
  ggtitle("Parameter estimation for xi under simple random sampling, \ndisplayed across 100 samples")
