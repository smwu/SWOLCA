% init_probit_params initializes priors and variables for the probit model 
...
%   indiv_lik_probit_class: matrix of indiv probit lik components used for posterior class membership; nx(k_max)
function probit_params = init_probit_params(data_vars, k_max, q_dem, mu_0, Sig_0)
    ...
    probit_params.indiv_lik_probit_class = zeros(data_vars.n, k_max); % Initialize matrix for individual probit likelihood contribution        
    for k = 1:k_max
       ...
       % Log-lik helps avoid rounding issues
       loglik_probit = data_vars.y .* log(Phi_temp) + (1-data_vars.y) .* log(1-Phi_temp); 
       % Probit likelihood for assigning indivs to class k
