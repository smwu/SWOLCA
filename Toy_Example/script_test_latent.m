...
assert(abs(probit_params.indiv_lik_probit_class(1,50) - 0.0547) <= tol)  %% failed 0.0671
assert(abs(probit_params.indiv_lik_probit_class(200,1) - 1) <= tol)  %% failed 8.4317e-06
...
for iter = 1:n_runs
    [MCMC_out, OFMM_params, probit_params] = wtd_update_MCMC_latent(MCMC_out, data_vars, ...
...
assert(abs(MCMC_out.pi(1,7) - 0.2076) <= tol)  %% failed 0.2340
assert(MCMC_out.c_i(1,1) == 22)
assert(abs(MCMC_out.theta(1,1,50,2) - 0.2108) <= tol)  %% failed 0.7195
assert(abs(MCMC_out.theta(1,1,1,1) - 0.6526) <= tol)   %% failed 0.3349
assert(abs(MCMC_out.xi(1,1) - 3.0935) <= tol)  %% failed 2.2851
assert(abs(MCMC_out.loglik(1) + 799.3670) <= tol)  %% failed 43.0420
assert(abs(OFMM_params.pi(35) - 0.0819) <= tol)  %% failed 0.3859
assert(abs(OFMM_params.theta(1,1,1) - 0.6057) <= tol)  %% failed 0.9070
assert(abs(OFMM_params.theta(4,50,2) - 0.1446) <= tol) 
assert(isequal(OFMM_params.c_i(1:5), [22; 35; 7; 7; 29]))  %% failed [22; 22; 7; 11; 35]
assert(OFMM_params.n_ci(7) == 38)  %% failed 48.2500
assert(abs(probit_params.xi(1) - 1.7924) <= tol)    %% failed 2.6435
assert(abs(probit_params.xi(52) - 2.3128) <= tol) 
assert(abs(probit_params.indiv_lik_probit_class(1,50) - 2.0193e-05) <= tol) 
assert(abs(probit_params.indiv_lik_probit_class(200,1) - 1) <= tol)  %% failed 0.0162
assert(abs(MCMC_out.pi(9,35) - 0.0819) <= tol)  %% failed 0.3859
assert(MCMC_out.c_i(9,200) == 7)
assert(abs(MCMC_out.theta(9,1,50,2) - 0.5777) <= tol)
assert(abs(MCMC_out.xi(9,52) - 2.3128) <= tol)
assert(abs(MCMC_out.loglik(9) + 537.2099) <= tol)  %% failed 348.9607
...
MCMC_out.z_i = zeros(n_runs / thin, data_vars.n);
MCMC_out.loglik = zeros(n_runs / thin, 1); 
[MCMC_out, OFMM_params, probit_params] = wtd_run_MCMC_latent(data_vars, ...
...
% many failed tests
assert(post_MCMC_out.k_med == 3)  %% failed 2
...
% DIC and AEBIC show up as negative. Why?
analysis.dic6
analysis.aebic
