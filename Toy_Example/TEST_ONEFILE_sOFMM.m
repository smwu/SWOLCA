...
% Convert xi from cell means to factor variable
if samp_data.true_Ci(1) ~= analysis.c_i(1)
    fact_xi = [analysis.xi_med(1) + analysis.xi_med(4), analysis.xi_med(2) + analysis.xi_med(4), analysis.xi_med(1) + analysis.xi_med(3), analysis.xi_med(2) + analysis.xi_med(3)];
else
    fact_xi = [analysis.xi_med(1) + analysis.xi_med(3), analysis.xi_med(2) + analysis.xi_med(3), analysis.xi_med(1) + analysis.xi_med(4), analysis.xi_med(2) + analysis.xi_med(4)];
end    
fact_xi

% sum abs dev and sum squared distance for xi's
sum(abs(sort(samp_data.true_xi) - sort(fact_xi)))
sum((sort(samp_data.true_xi) - sort(fact_xi)).^2) % take this over iterations to get MSE