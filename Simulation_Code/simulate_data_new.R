...
get_betas_c <- function(pi_mat, formula_c, V_unique) {
  # Check errors
  if (abs(sum(rowSums(pi_mat) - 1)) > 1e-15) {
...
#' @param save_path_full Boolean indicating if the full file name, including file 
#' type, is provided in `save_path`. Default is `FALSE`.
#' 
#' @details
...
#==================== Create population and sampling scenarios =================
...
for (samp_n in samp_n_seq) {
  samp_data_path <- paste0(wd, data_dir, "simdata_scen", scenario, "_iter", 
                           iter_pop, "_samp", samp_n, ".RData") 
  sim_data <- simulate_samp(sim_pop = sim_pop, samp_prop = 0.05, strat = FALSE,
                            strat_dist = c(0.5, 0.5), clust = FALSE,
                            samp_seed = samp_n, save_res = TRUE, 
                            save_path = samp_data_path,
                            save_path_full = TRUE)
...
# sim_pop <- simulate_pop(N = N, J = J, K = K, R = R, N_s = N_s,
#                         modal_theta_prob = modal_theta_prob,
#                         formula_c = formula_c, formula_x = formula_x_temp,
#                         formula_y = formula_y, beta_mat_c = beta_mat_c,
#                         beta_list_x = beta_list_x_temp, beta_vec_y = beta_vec_y,
#                         cluster_size = cluster_size,
#                         pop_seed = pop_seed, save_res = TRUE, 
#                         save_path = "Data/simpop_scen_test.RData",
#                         save_path_full = TRUE)
# 
# # Get sample, with no clustering
# sim_samp <- simulate_samp(sim_pop = sim_pop, samp_prop = 0.05, strat = TRUE,
#                           strat_dist = c(0.5, 0.5), clust = FALSE,
#                           samp_seed = 101, save_res = TRUE, 
#                           save_path = "Data/simdata_scen_test.RData",
#                           save_path_full = TRUE)
